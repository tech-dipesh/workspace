...
return {
  ...
  -- ── Mason: auto-install LSP servers + tools ─────────────
  {
    ...
    config = function(_, opts)
      ...
      -- Auto-install any missing tools after registry refresh
  ...
  -- ── mason-lspconfig: bridge mason → lspconfig ─────────────
  {
    ...
    opts = {
      ensure_installed = {
        ...
        "dockerls", "bashls",
        -- NOTE: ts_ls and clangd are added via their own setup calls below
      },
  ...
  -- ── nvim-lspconfig: configure each server ─────────────────
  {
    ...
    config = function()
      -- Neovim Lua dev setup (must run before lspconfig)
      require("neodev").setup({})

      -- Shared LSP capabilities (adds nvim-cmp completions)
      ...
      lsp.lua_ls.setup({
        ...
        settings = {
          Lua = {
            ...
            hint        = { enable = false },   -- NO inlay hints
      ...
      -- Using ts_ls (the official typescript-language-server)
      -- ALL inlay hints disabled as requested
      lsp.ts_ls.setup({
        capabilities = capabilities,
        on_attach    = on_attach,
        settings = {
