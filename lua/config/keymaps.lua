...
-- ══════════════════════════════════════════════════════════
--  SECTION A — YOUR ORIGINAL KEYMAPS (not touched)
-- ══════════════════════════════════════════════════════════
...
-- ── Terminal: informative title (no clock) ───────────────
-- Shows: project name | git branch | staged/modified counts | diagnostics
local _term_instances = {}   -- track all open terminals by ID

local function get_git_info(root)
	-- Branch name
	local branch = vim.fn.system(
		'git -C "' .. root .. '" rev-parse --abbrev-ref HEAD 2>NUL'
	):gsub("%s+", "")
	...
	for line in status:gmatch("[^\n]+") do
		...
		if xy:sub(1,1):match("[MADRCU]") then staged    = staged    + 1 end
		if xy:sub(2,2):match("[MD]")     then modified  = modified  + 1 end
		if xy == "??"                    then untracked = untracked + 1 end
	end
...
local function terminal_title()
	...
	local diag_str  = ""
	if diag_err  > 0 then diag_str = diag_str .. "  " .. diag_err  end
	if diag_warn > 0 then diag_str = diag_str .. "  " .. diag_warn end
...
-- Dashboard ───────────────────────────────────────────────
map("n", "<leader>h", function() Snacks.dashboard.open() end, { desc = "Dashboard" })
...
map("n", "<leader>wm", function()
	if vim.t._maximized then
		...
		vim.cmd("wincmd |"); vim.cmd("wincmd _")
		vim.t._maximized = true
	end
	vim.notify(vim.t._maximized and " Maximized" or " Restored",
	vim.log.levels.INFO, { timeout = 900 })
...
-- ── Quickfix ──────────────────────────────────────────────
map("n", "[q", function() pcall(vim.cmd, "cprev"); vim.cmd("norm! zz") end, { desc = "Prev quickfix" })
map("n", "]q", function() pcall(vim.cmd, "cnext"); vim.cmd("norm! zz") end, { desc = "Next quickfix" })

-- ── Toggle options ────────────────────────────────────────
local function toggle(opt_name, label)
	return function()
		vim.o[opt_name] = not vim.o[opt_name]
		vim.notify((vim.o[opt_name] and "ON" or "OFF") .. ": " .. label,
		vim.log.levels.INFO, { title = "Toggle", timeout = 1000 })
	end
end
...
map("n", "<leader>uf", function()
	...
	vim.notify((vim.g.autoformat and "ON" or "OFF") .. ": Format on save",
	vim.log.levels.INFO, { title = "Toggle", timeout = 1000 })
end, { desc = "Toggle format on save" })

-- ── Search & replace helpers ─────────────────────────────
-- Replace word under cursor in buffer
map("n", "<leader>rs", ":%s/<C-r><C-w>//gc<Left><Left><Left>",
{ desc = "Replace word under cursor" })
map("v", "<leader>rs", ":s///gc<Left><Left><Left><Left>",
...
map("n", "<leader>wc", function()
	...
	vim.notify(("Words: %d  Chars: %d  Lines: %d"):format(
		wc.words, wc.chars, vim.fn.line("$")),
		vim.log.levels.INFO, { title = "Count" })
